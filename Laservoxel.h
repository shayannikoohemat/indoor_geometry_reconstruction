...
/*    LaserPoints closed_door(LaserPoints vox_cnts, LaserPoints traj_points, uint min_dilation=1,double struct_element_a=1.0,
   ...
   LaserPoints export_all();
    ...
    LaserPoints export_voxel_centres(int min_points_in_vox);
    vector< vector < vector < int > > > export_vox_centres(int min_points_in_vox=1, char * output= (char *) "D://test//vox_centers.laser"); ///only exports the centres of voxels (where laserpoints are)
};
